...
from .update_host_record_form import GenericFormTemplate
...
# chaos will ensue.
@route(app, "/update_host_record/update_host_record_endpoint")
@util.workflow_permission_required("update_host_record_page")
@util.exception_catcher
def update_host_record_update_host_record_page():
    """
...
@route(app, "/update_host_record/form", methods=["POST"])
@util.workflow_permission_required("update_host_record_page")
@util.exception_catcher
def update_host_record_update_host_record_page_form():
    """
    ...
    if form.validate_on_submit():
        try:
            ...
            for ip in ip4_addresses:
                if (
                    str(configuration.get_ip4_address(ip).get_type()) == "None"
                    or ip in original_ip4
                ):
            ...
            g.user.logger.info(
                "Success - Host Record Modified - Object ID: " + str(host_record.get_id())
            )
            flash(
                "Success - Host Record Modified - Object ID: " + str(host_record.get_id()),
                "succeed",
...
@route(app, "/update_host_record/get_deploy_status", methods=["POST"])
@util.rest_workflow_permission_required("update_host_record_page")
