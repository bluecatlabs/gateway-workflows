...
class bulk_handler:
    ...
    def __init__(self, g, form_data):
        ...
        try:
            ...
            self.view = self.config.get_view(self.form_data.view.data)
        except PortalException:
            self.errors = 1
            g.user.logger.info("Unable to get configuration or view!")
            self.config = None
    ...
    def handle_row(self, key, row):
        ...
        try:
            ...
            return
        try:
            self.view.add_host_record(csv_record, [csv_ip], csv_ttl)
        except (Exception, PortalException, APIException) as e:
            self.errors = self.errors + 1
            self.results[key]['result'] = "Unable to add host record. %s" %str(e)
            return
    ...
    def go_no_go(self):
        ...
        elif self.config is None or self.view is None:
            return True, "Unable to find config or view"
        return True, "Success!"
