...
function load_col_model() {
    $.ajax({
        type: 'GET',
        url: '/zone_exporter/load_col_model',
        async: false
    })
    .done(function(data) {
        for (var i in data) {
...
$(document).ready(function()
{
	...
	grid.jqGrid({
	    url: '/zone_exporter/load_initial_data',
        datatype: 'json',
        colModel: colModel,
        jsonReader: {
            repeatitems: false,
	...
	grid.on('jqGridBeforeExpandTreeGridRow', function(event, rowid, record, children) {
	    ...
	    if (0 == children.length) {
		    $.ajax({url: '/zone_exporter/load_children_data/' + id + '/' + level})
                .done(function(data)
                {
            	    for (var i in data) {
	...
	download.click(function() {
	    ...
	    a.href = '/zone_exporter/load_file/' + rowid + '/' + format + '/' + contents;
	    a.click();
