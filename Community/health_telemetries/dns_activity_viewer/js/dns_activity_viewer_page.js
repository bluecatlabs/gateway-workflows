...
// Description: Health Telemetry DNS Activity Viewer Main Page JS
...
function construct_list(id) {
    ...
    $.ajax({
        type: 'GET',
        url: '/dns_activity_viewer/load_col_model',
        async: false
    })
    .done(function(data) {
        title = data.title;
    ...
    var grid = $('#' + id);
    grid.jqGrid({
        url: '/dns_activity_viewer/load_activities?start=0&count=' + buffer_size,
        datatype: 'json',
        colModel: colModel,
        height: 216,
        rowNum: 10000,
...
function update_activity_list() {
    var grid = $('#activity_list');
    $.ajax({
        type: "GET",
        url: '/dns_activity_viewer/load_activities',
        data: {
            start: 0,
            count: buffer_size
        },
...
function set_detail(key) {
    ...
    $.ajax({
        type: 'GET',
        url: '/dns_activity_viewer/get_activity/' + key,
        async: false
    })
    .done(function(data) {
...
$(document).ready(function() {
    var grid = construct_list('activity_list');
    grid.setGridParam({
    ...
    if ($('#auto_refresh').prop('checked')) {
        setInterval('update_activity_list()', 1000 * interval);
    }
    
    $('#auto_refresh').change(function() {
        if ($(this).prop('checked')) {
            refresh = setInterval('update_activity_list()', 1000 * interval);
        }
        else {
